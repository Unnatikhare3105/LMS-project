...
const addBookmarkController = async (req, res, next) => {
    try {
        ...
        const bookmark = await bookmarkService.addBookmark(req.user.userId, syllabusId, note);
        res.status(201).json({
            success: true,
...
const updateBookmarkNoteController = async (req, res, next) => {
    try {
        ...
        const updated = await bookmarkService.updateNote(bookmarkId, req.user.userId, note);
