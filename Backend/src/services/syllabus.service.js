...
exports.deleteSyllabus = exports.updateSyllabusContent = exports.getSyllabusByPublicId = exports.getAllTopics = exports.generateFullContent = exports.generateContentAsVideo = exports.generateContentAsText = void 0;
...
const generateContentAsText = async ({ topic, userId }) => {
    ...
    if (existing) {
        ...
        if (!updated)
            throw new customError_1.default('Failed to update syllabus.', 500);
        syllabus = updated;
    }
    else {
...
const generateContentAsVideo = async ({ topic, userId }) => {
    const existing = await syllabusRepo.findSyllabusByUserAndTopic(userId, topic);
    if (existing && existing.videoLinks?.length)
        return existing;
...
const generateFullContent = async ({ topic, userId }) => {
    const existing = await syllabusRepo.findSyllabusByUserAndTopic(userId, topic);
    if (existing && existing.content && existing.videoLinks?.length)
        return existing;
    const [content, videos, references] = await Promise.all([
        aiService.getDetailedExplanation(topic),
        aiService.getVideoLinks(topic),
...
exports.generateFullContent = generateFullContent;
...
const getSyllabusByPublicId = async (publicId, userId) => {
    ...
    if (syllabus.userId.toString() !== userId)
        throw new customError_1.default('Forbidden.', 403);
    return syllabus;
};
exports.getSyllabusByPublicId = getSyllabusByPublicId;
// ─── Mutations ────────────────────────────────────────────────────────────────
const updateSyllabusContent = async (publicId, content, userId) => {
...
const deleteSyllabus = async (publicId, userId) => {
    const existing = await syllabusRepo.findSyllabusBySyllabusId(publicId);
    if (!existing)
        throw new customError_1.default('Syllabus not found.', 404);
    if (existing.userId.toString() !== userId)
