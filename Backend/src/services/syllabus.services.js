...
import * as AIService from './openAI.services.js';

export const generateContentAsText = async ({ topic, userId }) => {
  try {
...
export const generateContentAsVideo = async ({ topic, userId }) => {
  if (!topic) {
...
export const getall = async ({ userId }) => {
  try {
    const topics = await syllabusModel.find({ userId }).select('topic content -_id');
...
export const updateSyllabus = async ({ syllabusId, content }) => {
  if (!syllabusId) {
    throw new CustomError("Syllabus ID is required", 400);
  }
  if (!content) {
