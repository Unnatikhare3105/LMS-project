...
const recordActivity = async (userId) => {
    ...
    const user = await user_model_1.default.findOne({ userId }).exec();
    if (!user)
        return;
...
const incrementQuizCount = async (userId) => {
    await user_model_1.default.findOneAndUpdate({ userId }, { $inc: { totalQuizzesTaken: 1 } }).exec();
};
exports.incrementQuizCount = incrementQuizCount;
const incrementTopicCount = async (userId) => {
    await user_model_1.default.findOneAndUpdate({ userId }, { $inc: { totalTopicsSearched: 1 } }).exec();
};
